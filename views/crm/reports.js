...
import { esc, showError, toast, openModal, closeModal, loadingSkeleton, formatDate } from '../../js/ui.js';
import { parseCSV, exportCSV } from '../../js/csv.js';
import { navigate } from '../../js/router.js';

// Ported from the rtcompu CRM line onto the gstack CRM. gstack has no
// views/crm/common.js or js/audit.js, and its js/csv.js keeps header case, so
// the few helpers those provided are inlined here.
const canManageData = () => ['owner', 'admin', 'manager'].includes(getMembership()?.role);
async function fetchOrgUsers() {
  const { data, error } = await sb.from('users').select('id, full_name');
  if (error) return [];
  return data || [];
}
// crm_report_rows keys are lower-cased so new imports match existing rows and
// the content-hash dedup; parseCSV preserves header case, so lower-case here.
const parseCsv = (text) => parseCSV(text).map((o) =>
  Object.fromEntries(Object.entries(o).map(([k, v]) => [k.trim().toLowerCase(), v])));
// No audit log on gstack — keep call sites intact with a no-op.
const logAction = async () => {};
const downloadCsv = (filename, rows) => exportCSV(filename, rows, Object.keys((rows && rows[0]) || {}));
  ...
  function openImport() {
    ...
    wrap.innerHTML = `
      <div style="display:grid;gap:var(--space-3)">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-3)" id="rep-cols-wrap" style="display:none">
          <div>
