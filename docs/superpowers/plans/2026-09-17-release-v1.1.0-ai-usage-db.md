...
First re-check nothing was created since: `select to_regclass('public.ai_usage'), to_regclass('public.platform_admins'), (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public' and p.proname in ('is_platform_admin','ai_is_org_admin_of','ai_org_tz','ai_quota_check','ai_record_usage','ai_bot_check','platform_set_org_quota','ai_set_user_limit','ai_clear_flag','ai_my_usage','ai_usage_report','ai_org_usage','platform_org_usage','platform_org_detail','ai_seed_new_org'));` → `null, null, 0`.

Supabase `apply_migration` with `project_id = nburswxjpukntgdwuyme`, `name = ai_usage_quotas`, `query` = the exact bytes of `supabase/pending/ai_usage_quotas.sql` as committed, read with `git show HEAD:supabase/pending/ai_usage_quotas.sql` (never the working-tree copy: `core.autocrlf=true` adds CR).
...
```
Expected: `quotas = 5, defaults = 5, platform_admins = 1, usage_rows = 0`. Also run Supabase `get_advisors` (security). Expected and intended, not failures: `rls_enabled_no_policy` on `platform_admins` (spec §4.2), and SECURITY DEFINER functions executable by `authenticated` (`platform_set_org_quota`, `ai_set_user_limit`, `ai_clear_flag`, `ai_my_usage`, `ai_org_usage`, `platform_org_usage`, `platform_org_detail`, `is_platform_admin`, `ai_is_org_admin_of` — each re-checks the caller). Anything else naming an `ai_*`, `platform_*` or `is_platform_admin` object is a finding: stop and report, do not change the database.

The test suite is one-shot: it assumes the usage tables are empty and every org still has its seeded quota, so run Step 7 immediately after the apply and before any AI usage exists.
