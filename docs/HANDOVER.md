# Atllanta — Full Development Handover (Deep Reference)

> **What this file is:** The exhaustive reference for Atllanta — the original architecture blueprint, full DB schema, design tokens, event recipes, and phase plan. It is NOT auto-loaded into every session (that lives in the lean `/CLAUDE.md`). Open this only when you need the deep detail. Where this doc and the actual repo disagree, the repo wins: `supabase/migrations/` is the real schema, `css/tokens.css` is the real design tokens, and the `api/`, `js/`, `views/` trees are the real file layout.
