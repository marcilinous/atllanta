 ...
 *
 * IMPORTANT: call this WITHOUT `await` — it is fire-and-forget by design. It
 * must never sit on the request's critical path: if Langfuse is slow or
 * misconfigured, awaiting it would add latency to (or, across a loop, time
 * out) the Groq endpoint. It always resolves and never throws, so an
 * un-awaited call can never produce an unhandled rejection. The trace POST
 * piggybacks on the request that continues after it (DB writes, response),
 * which normally keeps the function alive long enough to deliver it.
 *
 * @param {object} o
