...
export async function runAI({ caller, feature, messages, maxTokens, temperature = 0.2, metadata = {} }) {
  const { db, orgId, userId, role } = caller;
  ...
  if (quota.allowed !== true) {
    ...
    const canSeeOrg = role === "owner" || role === "admin";
    return {
      ok: false,
      status: 429,
      body: {
        ...
        quota: {
          ...
          ...(canSeeOrg ? { org_used: quota.org_used, org_quota: quota.org_quota } : {}),
  ...
  const failGroq = async (detail) => {
    ...
    logGroqGeneration({
      ...
      userId, metadata: { ...metadata, org_id: orgId, feature }, modelParameters, level: "ERROR", statusMessage: detail,
    });
  ...
  logGroqGeneration({
    ...
    userId, metadata: { ...metadata, org_id: orgId, feature }, modelParameters,
  });
