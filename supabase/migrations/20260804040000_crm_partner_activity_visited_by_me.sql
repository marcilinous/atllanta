-- ============================================================
-- crm_partner_activity(): add visited_by_me / called_by_me
-- ============================================================
-- Activity attributed to the viewing user (report row person_user_id =
-- auth.uid()), so a BDE's own field visits map onto their partners in
-- 'My targets'. visited/called (by anyone) are kept for the manager views.
-- ============================================================
...
BEGIN
  ...
  vis AS (
    SELECT rr.account_id AS acct_id, bool_or(rr.person_user_id = auth.uid()) AS mine
    FROM crm_report_rows rr JOIN crm_report_imports i ON i.id=rr.import_id
