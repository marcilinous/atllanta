-- ============================================================
-- crm_partner_activity(): add visit/call counts (numbers, not icons)
-- ============================================================
-- visits_me = number of visits by the viewing user to the partner;
-- calls_total = number of calls to the partner (by anyone / telecaller).
-- Lets the UI show numeric touch columns instead of icons.
-- ============================================================
...
BEGIN
  ...
  vis AS (
    SELECT rr.account_id AS acct_id,
      ...
      count(*) FILTER (WHERE rr.person_user_id = auth.uid())::int AS my_count
    FROM crm_report_rows rr JOIN crm_report_imports i ON i.id=rr.import_id
