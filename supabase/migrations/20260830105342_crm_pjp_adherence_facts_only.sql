-- Per-visit plan tagging. Value carried is the partner's real billed rupees
-- over the last 12 months, taken once per partner - not a weighted score.
CREATE FUNCTION crm_pjp_visit_adherence(p_from date, p_to date)
RETURNS TABLE(
...
-- Weekly rollup: planned days, visits that landed in the planned territory,
-- and the real billed value on each side of the plan.
CREATE FUNCTION crm_pjp_adherence(p_from date, p_to date)
RETURNS TABLE(
