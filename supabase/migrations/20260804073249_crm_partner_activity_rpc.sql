-- Per-partner activity used to derive coverage opportunities.
-- TP/TSS counts split by fiscal year (Apr–Mar): CFY = current, LFY = prior.
-- LFY populates once last year's Activation report is uploaded (classified
-- by activation date). Visit/call flags from the visit / telecalling reports.
-- Scoped to caller visibility.
