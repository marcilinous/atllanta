-- Per-owner partner coverage, scoped to what the caller can see
-- (admin: all; manager: their reporting subtree; rep: own).
...
-- Untouched (no linked report activity) partners for one owner, scoped.
