-- Add an optional activation-date window to the sales aggregation.
-- Dates are ISO 'YYYY-MM-DD' in the report, so string comparison is correct
-- and avoids cast errors on any stray value. Empty string = unbounded.
DROP FUNCTION IF EXISTS crm_sales_by(text);
DROP FUNCTION IF EXISTS crm_sales_by(text, text, text);

CREATE OR REPLACE FUNCTION crm_sales_by(p_dim text, p_from text DEFAULT '', p_to text DEFAULT '')
...
REVOKE ALL ON FUNCTION crm_sales_by(text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION crm_sales_by(text, text, text) TO authenticated;